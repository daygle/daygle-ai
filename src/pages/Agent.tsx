...
export function AgentPage() {
  ...
  const refreshHistory = useCallback(() => {
    // Only agent-origin conversations belong in the Agent page's history;
    // plain Chat page sessions live in /chat.
    listChatSessions(agentUrl, "agent").then(setHistory).catch(() => {});
  }, [agentUrl]);
  ...
  useEffect(() => {
    ...
    try {
      // Agent sessions resume from their own key so they never pick up the
      // Chat page's last conversation (and vice versa).
      lastId = localStorage.getItem("daygle.agentSessionId");
    } catch {
      lastId = null;
    }
  ...
  function rememberSession(id: string | null) {
    try {
      if (id) localStorage.setItem("daygle.agentSessionId", id);
      else localStorage.removeItem("daygle.agentSessionId");
    } catch {
  ...
  async function handleConnect() {
    ...
    try {
      ...
      const providerConfig: ProviderConfig | undefined = providerKind === "openai"
        ...
        : undefined;
      const session = await createChatSession(agentUrl, repo, model, LOCAL_OLLAMA_URL, loadGenOptions(), providerConfig, "agent");
  ...
  async function resumeChat(id: string, opts?: { silent?: boolean }) {
    ...
    try {
      const chat = await getChatSession(agentUrl, id);
      if (requestId !== resumeRequestRef.current) return;
      // A conversation owned by the Chat page doesn't belong in the Agent page.
      if (chat.origin === "chat") {
        if (!opts?.silent) {
          setMessages([{ id: uid(), role: "assistant", content: "That conversation lives on the Chat page." }]);
          setConnected(true);
        } else {
