...
function buildBody(task: string, summary: string, files: string[]): string {
  return [
    `## ${task}`,
    "",
    summary,
    ...
    "",
    "---",
    "_Generated by the daygle agent (local Ollama + GitHub CLI)._",
  ].join("\n");
}

async function executeJob(job: Job): Promise<void> {
  const emit = (event: AgentEvent) => publish(job, event);
  ...
  let workDir = "";
  try {
    const appMode = githubAppAvailable();
    ...
    const base = job.baseBranch || (await detectDefaultBranch(job.repoUrl, token));
    const branch = `daygle/${slugify(job.task)}-${Date.now().toString(36)}`;
    ...
    if (changed.length > 0) {
      ...
      const body = buildBody(job.task, summary, changed);
      let prUrl: string;
      if (appMode && token) {
